...
def plot_boxplots(
    ...
    y_axis_label=None, y_axis_ticks=None
):
    ...
    :param y_axis_label: str - the y-axis label
    :return: the header axes
    """
    ...
    for idx, source in enumerate(sources):
        ax = fig.add_subplot(gs[0 if title is None else 1, idx], sharey=ax)
        if title is None and ax_header is None:
            ax_header = ax
        ...
        if y_axis_label is not None and idx == 0:
...
def plot_kmer_similarity(
    ...
    tick_pad_x=-3,
    label_pack_level=2,
    title="k-mer similarity vs. k-mer length"
):
    ...
    if title is not None:
        ...
        rm_axes_elements(ax_header, "plain")
        ax_header.text(
            0.5, 1.0, title, fontsize=16 * font_scale, ha="center", va="top"
        )
        ax = fig.add_subplot(gs[1, :])
    else:
        gs = area.subgridspec(
            1, 1, width_ratios=[1], height_ratios=[1], wspace=0.0, hspace=0.0
        )
        fig = plt.gcf()
